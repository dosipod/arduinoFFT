...
{ // vM is half the size of vReal and vImag
	for (uint16_t i = 0; i < this->_samples; i++) {
		this->_vReal[i] = sqrtf(sq(this->_vReal[i]) + sq(this->_vImag[i]));
	}
}
...
{	// vM is half the size of vReal and vImag
	#warning("This method is deprecated and may be removed on future revisions.")
	for (uint16_t i = 0; i < samples; i++) {
		vReal[i] = sqrtf(sq(vReal[i]) + sq(vImag[i]));
	}
}
...
// The weighing function is symetric; half the weighs are recorded
	...
	for (uint16_t i = 0; i < (this->_samples >> 1); i++) {
		...
		case FFT_WIN_TYP_TRIANGLE: // triangle (Bartlett)
			...
			weighingFactor = 1.0f - ((2.0f * fabsf(indexMinusOne - (samplesMinusOne / 2.0f))) / samplesMinusOne);
			#else
...
void arduinoFFT::MajorPeak(float *f, float *v)
{
	...
	*f = interpolatedX;
	#if defined(ESP8266) || defined(ESP32)
	*v = fabsf(this->_vReal[IndexOfMaxY - 1] - (2.0f * this->_vReal[IndexOfMaxY]) + this->_vReal[IndexOfMaxY + 1]);
	#else
...
void arduinoFFT::MajorPeak(float *vD, uint16_t samples, float samplingFrequency, float *f, float *v)
{
	...
	*f = interpolatedX;
	#if defined(ESP8266) || defined(ESP32)
	*v = fabsf(vD[IndexOfMaxY - 1] - (2.0f * vD[IndexOfMaxY]) + vD[IndexOfMaxY + 1]);
	#else
